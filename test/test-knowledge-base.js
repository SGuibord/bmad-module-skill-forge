/**
 * Knowledge Base Validation Tests - SKF Module
 *
 * Tests skf-knowledge-index.csv parsing, fragment existence, tag selection,
...
function runTests() {
  ...
  console.log('SKF Knowledge Base Tests');
  console.log(`========================================${colors.reset}\n`);

  const projectRoot = path.join(__dirname, '..');
  const kbRoot = path.join(projectRoot, 'src', 'knowledge');
  const indexPath = path.join(kbRoot, 'skf-knowledge-index.csv');
  ...
  try {
    ...
    assert(records.length === 12, 'skf-knowledge-index.csv has 12 fragment records', `Found ${records.length}`);
    ...
    assert(missingFields.length === 0, 'skf-knowledge-index.csv has required columns', missingFields.join(', '));
    ...
    assert(false, 'skf-knowledge-index.csv parsed successfully', error.message);
  }
  ...
  if (records.length > 0) {
    ...
    assert(false, 'fragment records loaded', 'No records found in skf-knowledge-index.csv');
  }
  ...
  for (const fileName of mdFiles) {
    ...
    for (const match of linkMatches) {
      ...
      const resolved = resolveFragmentLink(path.join(projectRoot, 'src'), href);
