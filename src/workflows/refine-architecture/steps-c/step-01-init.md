...
- Confirm the file exists and is readable
- If missing: "VS report not found at `{path}`. Proceeding without VS report — issue detection will rely on skill data only."
- Store VS report availability as `vs_report_available: true|false`
...
| **VS Report** | {vs_report_name or 'Not provided — issue detection will use skill data only'} |
| **Skills Loaded** | {count} |
