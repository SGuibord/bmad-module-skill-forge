...
- Rationale: a blocked integration represents a fundamental architectural incompatibility

**CONDITIONALLY FEASIBLE:**
- All integrations are Verified or Plausible, BUT requirements have any **Not Addressed** items → overall verdict is CONDITIONALLY FEASIBLE
- Any integration is **Risky** (but none Blocked) → overall verdict is CONDITIONALLY FEASIBLE
- Rationale: the stack can work but has gaps or risks that must be addressed

**FEASIBLE:**
- All integrations are Verified or Plausible AND requirements are all Fulfilled or Partially Fulfilled (or requirements pass was skipped) AND no Blocked or Risky integrations → overall verdict is FEASIBLE
- Rationale: the stack can support the architecture as described
...
- If type incompatibility → "Add a serialization/conversion layer between `{lib_a}` ({format_a}) and `{lib_b}` ({format_b})."
...
- If fundamental incompatibility → "Replace `{blocked_lib}` with `{suggested_alternative}` or redesign the integration path."
...
Search `{forge_data_folder}` for an existing feasibility report (any file matching `feasibility-report-{project_name}*.md` other than the current one).
...
- FEASIBLE → "Proceed to **[RA] Refine Architecture** to produce an implementation-ready architecture, then **[SS]** to compose your stack skill."
- CONDITIONALLY FEASIBLE → "Address the {N} recommendations above, then re-run **[VS]**. Once all clear, proceed to **[RA]**."
...
- Set `coverage_count`, `integrations_verified_count`, `integrations_risky_count`, `integrations_blocked_count`, `requirements_fulfilled_count`
